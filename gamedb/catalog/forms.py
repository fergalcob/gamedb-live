...
class ReviewBox(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(Div("title", "comment"))
...
class updateForm(forms.Form):
    ...
    first_name = forms.CharField(
        max_length=50, label="First Name", required=False
    )
...
class existingLists(forms.Form):
    add_to_list = UserModelChoiceField(
        queryset=Game_List.objects.all(), to_field_name="id"
    )


class removeFromLists(forms.Form):
    remove_from_list = UserModelChoiceField(
        queryset=Game_List.objects.all(), to_field_name="id"
    )
